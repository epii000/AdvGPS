...
#     box2d = torch.tensor(box2d).clone().detach().requires_grad_(True)
...
#     return torch.tensor(projected_points[:, :3])
...
def project_points_by_matrix_torch(points, transformation_matrix):
    ...
    transformation_matrix, is_numpy = \
        common_utils.check_numpy_to_torch(transformation_matrix)
...
def project_world_objects(object_dict,
    ...
    #     #print(globals.flag_num, "  flag:  ", globals.flag, "  the first  T: ", globals.T_learnable)
    ...
    #     #print(globals.flag_num, " flag:  ", globals.flag, "  learnable T: ", transformation_matrix)
    ...
    for object_id, object_content in object_dict.items():
        ...
        #print("object_pose", type(object_pose))

        object2lidar = x1_to_x2(object_pose, transformation_matrix)

        
        #object2lidar = torch.tensor(object2lidar)
        # shape (3, 8)
        ...
        bbx_lidar = torch.from_numpy(bbx_lidar)
        # print("bbx_lidar: ", bbx_lidar)
        if bbx_lidar.shape[0] > 0:
            # bbx_lidar = torch.tensor(bbx_lidar)
            # bbx_lidar =  torch.from_numpy(bbx_lidar)
            # print("bbx_lidar: ", bbx_lidar)
