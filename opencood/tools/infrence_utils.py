...
def save_prediction_gt(pred_tensor, gt_tensor, pcd, timestamp,vis_attacked_lidar, save_path):
    """
    ...
    vis_attacked_lidar = torch_tensor_to_numpy(vis_attacked_lidar)

    # np.save(os.path.join(save_path, '%04d_pcd.npy' % timestamp), pcd_np)
    np.save(os.path.join(save_path, '%04d_attacked_pcd.npy' % timestamp), vis_attacked_lidar)
