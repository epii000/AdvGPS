...
def train_parser():
    ...
    parser.add_argument("--hypes_yaml", type=str,default="/home/jinlongli/1.Detection_Set/V2V_Attack/opencood/hypes_yaml/voxelnet_intermediate_fusion.yaml",#required=True, #
                        help='data generation yaml file needed ')
    parser.add_argument('--model_dir', default='',
                        help='Continued training path')
    parser.add_argument('--model', default='/home/jinlongli/2.model_saved/3.Attack_V2V2023/0.attack_model/attfuse_voxelNet_finetune_2023_08_23_22/net_epoch30.pth',
...
def main():
    ...
    print('MMD Attack testing start')
    ...
    def shift_loss(source_pose,attacked_pose):
        num = source_pose.shape[0]
        # pdb.set_trace()
        distance = torch.tensor(0).type(torch.float32)
        ...
        for i in range(1,num):
            ...
            cav_location = cav_in_ego[0:3,3]
            attacked_cav_location = attacked_cav_in_ego[0:3,3]
            # v1 = math.sqrt((cav_location[0]-attacked_cav_location[0])**2 + (cav_location[1]-attacked_cav_location[1])**2 + (cav_location[2]-attacked_cav_location[2])**2)
            # v2 = torch.sqrt((cav_location[0]-attacked_cav_location[0])**2 + (cav_location[1]-attacked_cav_location[1])**2 + (cav_location[2]-attacked_cav_location[2])**2)
            distance += torch.sqrt((cav_location[0]-attacked_cav_location[0])**2 + (cav_location[1]-attacked_cav_location[1])**2 + (cav_location[2]-attacked_cav_location[2])**2)
    ...
    for epoch in range(init_epoch, max(epoches, init_epoch)):
        ...
        #mmdloss_1 = MMDLoss(768.0)
        mmdloss_2 = MMDLoss(128.0)
        mmdloss_3 = MMDLoss(128.0)
        mmdloss_4 = MMDLoss(2.0)
        mmdloss_5 = MMDLoss(14.0)
        for batch_i, batch_data in enumerate(data_loader):
            ...
            import copy
            ###perfect data
            batch_data_perfect_init =  copy.deepcopy(batch_data)
            batch_data_perfect_init['ego'].pop('base_data_dict')
            batch_data_perfect_init['ego'].pop('processered_lidar_np')
            batch_data_perfect_init['ego'].pop('transformation_matrix')
            ...
            #learn
            add_pose = torch.zeros_like(pose) #torch.ones_like(pose)#
            ...
            for inter in range(num_iter):
                ...
                if First_flag:
                    ...
                    for index in range(6):
                        if Is_learn[index]:
                            ...
                            v2 = []
                            v2.append(torch.tensor([[0,0]]))
                            for i in range(1,add_pose.shape[0]):
                                value=pose_limit[index]/10
                                v2.append(torch.tensor([[random.uniform(-value,value),0]]))
                ...
                batch_data_perfect = train_utils.to_device(batch_data_perfect_init, device)
                batch_data = train_utils.to_device(batch_data, device)

                ouput_dict_perfect = model(batch_data_perfect['ego'])
                ouput_dict_attack = model(batch_data['ego'])



                final_loss = criterion(ouput_dict_attack, batch_data['ego']['label_dict'])
                
                final_loss_2 = mmdloss_2(ouput_dict_perfect['out_feature_2'],ouput_dict_attack['out_feature_2'])

                final_loss_3 = mmdloss_3(ouput_dict_perfect['out_feature_3'],ouput_dict_attack['out_feature_3'])

                final_loss_4 = mmdloss_4(ouput_dict_perfect['psm'],ouput_dict_attack['psm'])

                final_loss_5 = mmdloss_5(ouput_dict_perfect['rm'],ouput_dict_attack['rm'])
                
                point_shift_loss = shift_loss(pose,pose+add_pose)

                # if point_shift_loss == 0:
                #     loss_attack = final_loss
                # else:
                #     loss_attack  = final_loss + 10*point_shift_loss 
                if inter == 0:
                    loss_attack = final_loss_2 + final_loss_3 + final_loss_4 + final_loss_5 + final_loss
                else:
                    loss_attack = final_loss_2 + final_loss_3 + final_loss_4 + final_loss_5 + point_shift_loss + final_loss

                
                #print('mmd loss print:', loss_attack)
            ...
            print('sample index: '+str(batch_i)+'   distance: '+str(distance)+'\n')
