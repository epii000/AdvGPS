...
def train_parser():
    ...
    parser.add_argument("--hypes_yaml", type=str,default="/home/jinlongli/1.Detection_Set/V2V_Attack/opencood/hypes_yaml/point_pillar_intermediate_V2VAM.yaml",#required=True, #
                        help='data generation yaml file needed ')
    parser.add_argument('--model_dir', default='',
                        help='Continued training path')
    parser.add_argument('--model', default="/home/jinlongli/2.model_saved/3.Attack_V2V2023/0.attack_model/V2VAM_finetune_voxel_2023_07_21_07/net_epoch40.pth",
                        help='for fine-tuned training path')
    parser.add_argument('--attacked_pose_path',default="/home/jinlongli/2.model_saved/3.Attack_V2V2023/1.attfuse_random_xy_2023_08_17_14/attacked_pose",
                        help='folder path of attacked pose')
...
def main():
    ...
    for epoch in range(init_epoch, max(epoches, init_epoch)):
        ...
        attacked_pose_path = opt.attacked_pose_path
        ...
        for batch_i, batch_data in enumerate(data_loader):
            ...
            #load attacked pose
            add_pose_full_path = os.path.join(attacked_pose_path,str(batch_i)+'.npy')
            car_id = list(batch_data['ego']['base_data_dict'][0])
            save_data = torch.load(add_pose_full_path)#{'car_id':car_id,'add_pose':add_pose}
            if car_id != save_data['car_id']:
                print('car_id not match!!!')
                break
                
            add_pose = save_data['add_pose']
            ...
            # v1 = batch_data['ego']['processed_lidar']
            # v2 = processed_feature
            # if v1 ==  v2:
            #     print('same')
            # else:
            #     print('unsame')
            ...
            if hypes['Attack_params']['npy_save'] and (batch_i)%25==0:
