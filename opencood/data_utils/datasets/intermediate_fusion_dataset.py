...
class IntermediateFusionDataset(basedataset.BaseDataset):
    def __init__(self, params, visualize, train=True, isSim=False):
        ...
        ###baolu
        # self.noise_matrix = nn.Parameter(torch.zeros((4,4)))
    ...
    #         return [self.convert_to_tensor(item) for item in data]
    ...
    def __getitem__(self, idx):
        ...
        for i, (cav_id, selected_cav_base) in enumerate(base_data_dict.items()):
            ...
            #TODO:jinlong
            #object_stack.append(torch.tensor(selected_cav_processed['object_bbx_center']))
            ...
            #TODO:baolu
            ego_pose.append(selected_cav_processed['ego_pose'])  ###
            transformation_matrix.append(selected_cav_processed['transformation_matrix'])  ###
            source_features.append(selected_cav_processed['source_features'])  ###
            processered_lidar_np.append(selected_cav_processed['processered_lidar_np'])
        ...
        # object_stack = torch.tensor(object_stack)
        #pdb.set_trace()
        # object_stack = self.convert_to_tensor(object_stack)
        #object_stack = torch.vstack(object_stack)
        ...
        ###baolu
        #object_bbx_center = torch.tensor(object_bbx_center)
        ...
        processed_data_dict['ego'].update(
            {'object_bbx_center': object_bbx_center,
             ...
             'processered_lidar_np':processered_lidar_np}) #TODO:baolu
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose):
        ...
        # print("not self.isSim", not self.isSim)
        ...
        processered_lidar_np = np.copy(lidar_np)
        selected_cav_processed.update(
            {'processered_lidar_np': processered_lidar_np
             })
        ...
        selected_cav_processed.update(
            {'object_bbx_center': object_bbx_center_valid,
             ...
             'velocity': velocity,
             'transformation_matrix':transformation_matrix, ###
             'source_features':selected_cav_base['lidar_np'], ###
             'ego_pose':ego_pose ###
             })
    ...
    def collate_batch_train(self, batch):
        ...
        for i in range(len(batch)):
            ...
            anchor_box.append(ego_dict['anchor_box'])
            ...
            ###baolu
            transformation_matrix.append(ego_dict['transformation_matrix'])
            base_data_dict.append(ego_dict['base_data_dict'])
            processered_lidar_np.append(ego_dict['processered_lidar_np'])
            if self.visualize:
        ...
        ###baolu
        #object_bbx_center = torch.tensor([item.cpu().detach().numpy() for item in object_bbx_center])
        ...
        # output_dict['ego'].update({'anchor_box':anchor_box,
        #                             'object_bbx_center': object_bbx_center,
