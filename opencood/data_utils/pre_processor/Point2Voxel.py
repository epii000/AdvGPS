...
class Point2Voxel(nn.Module):
    def __init__(self, vsize_xyz, coors_range_xyz, max_num_points_per_voxel, num_point_features):
        super(Point2Voxel, self).__init__()
        """
        初始化 Point2VoxelCPU3d 类。

        Parameters:
            vsize_xyz (list): 体素大小，即 x、y 和 z 方向上的体素数量。
            coors_range_xyz (list): 坐标范围，即 x、y 和 z 方向上的坐标范围。
            max_num_points_per_voxel (int): 每个体素中的最大点数。
            num_point_features (int): 点云特征的数量。
        """
        # 初始化参数
        self.vsize_xyz = torch.tensor(vsize_xyz)
        self.coors_range_xyz = torch.tensor(coors_range_xyz)
    ...
    def forward(self, points):
        ...
        Parameters:
            points (Tensor): 输入的点云张量，形状为 (N, C)。

        Returns:
            voxels (Tensor): 生成的体素张量，形状为 (V, M, C)。
            voxel_coords (Tensor): 体素坐标张量，形状为 (V, 3)。
            voxel_counts (Tensor): 体素点数计数张量，形状为 (V,)。
        """
        voxel_coords = ((points[:, :3] - self.coors_range_xyz[:3]) / (self.coors_range_xyz[3:] - self.coors_range_xyz[:3]) * self.vsize_xyz).to(torch.int32)
        voxel_coords = voxel_coords[:, [2, 1, 0]]  # 调整坐标顺序
        voxel_coords, inv_ind, voxel_counts = torch.unique(voxel_coords, dim=0, return_inverse=True, return_counts=True)

        voxels = torch.zeros((len(voxel_coords), self.max_num_points_per_voxel, self.num_point_features))
        for i in range(len(voxel_coords)):
            pts = points[inv_ind == i]
            if len(pts) > self.max_num_points_per_voxel:
                pts = pts[:self.max_num_points_per_voxel]
                voxel_counts[i] = self.max_num_points_per_voxel
            voxels[i, :len(pts)] = pts

        return voxels, voxel_coords, voxel_counts
    


if  __name__=='__main__':

    vsize_xyz = [128, 128, 32]
    coors_range_xyz = [0, -40, -3, 70.4, 40, 1]
    max_num_points_per_voxel = 10
    num_point_features = 4

    voxel_generator = Point2Voxel(vsize_xyz, coors_range_xyz, max_num_points_per_voxel, num_point_features)

    points = torch.randn(1000, num_point_features)  # 替换为实际的点云数据

    voxels, voxel_coords, voxel_counts = voxel_generator(points)

    print("Voxels shape:", voxels.shape)
    print("Voxel coords shape:", voxel_coords.shape)
    print("Voxel counts shape:", voxel_counts.shape)
