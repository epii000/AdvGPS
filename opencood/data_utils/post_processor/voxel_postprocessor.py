...
class VoxelPostprocessor(BasePostprocessor):
    ...
    def generate_label(self, **kwargs):
        ...
        # (H*W*anchor_n)
        iou = bbox_overlaps(
            np.ascontiguousarray(anchors_standup_2d).astype(np.float32),  #.detach().numpy()
            np.ascontiguousarray(gt_standup_2d).astype(np.float32),  #.detach().numpy()
        )
        ...
        # gt_box_center = gt_box_center.detach().numpy()generate_object_center
